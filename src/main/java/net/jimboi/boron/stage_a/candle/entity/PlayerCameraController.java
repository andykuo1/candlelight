...
/**
 * Created by Andy on 7/30/17.
 */
public class PlayerCameraController extends CameraController implements InputLayer
{
	...
	public PlayerCameraController(EntityPlayer player)
	{
	...
	@Override
	public PlayerCameraController setTarget(Transform3 target)
	{
		return (PlayerCameraController) super.setTarget(target);
	}

	public PlayerCameraController setSpeed(float speed)
	{
		this.speed = speed;
		return this;
	}

	public PlayerCameraController setDistanceToTarget(float dist)
	{
	...
	public PlayerCameraController setMaxDistanceToTarget(float maxDist)
	{
		this.maxDistToTarget = maxDist;
		return this;
	}

	public PlayerCameraController setMinDistanceToTarget(float minDist)
	{
	...
	public boolean onCameraUpdate(Camera camera, Transform3 cameraTransform, double delta)
	{
		...
		float dr = this.right * speed;
		float df = -this.forward * speed;

		if (!this.player.getWorld().getDungeonHandler().intersects(this.target.position.x + dr, this.target.position.y + df))
		{
