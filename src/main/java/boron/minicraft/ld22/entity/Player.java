...
import boron.minicraft.ld22.entity.particle.TextParticle;
...
import boron.minicraft.ld22.item.PowerGloveItem;
import boron.minicraft.ld22.level.Level;
import boron.minicraft.ld22.level.tile.Tile;
import boron.minicraft.ld22.screen.InventoryMenu;
...
public class Player extends Mob {
	...
	public void tick() {
		...
		int xa = 0;
		int ya = 0;
		if (input.isInputDown("up")) ya--;
		if (input.isInputDown("down")) ya++;
		if (input.isInputDown("left")) xa--;
		if (input.isInputDown("right")) xa++;
		...
		if (input.isInputReleased("attack")) {
			if (stamina == 0) {
