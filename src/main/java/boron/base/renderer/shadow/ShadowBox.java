package boron.base.renderer.shadow;
...
public class ShadowBox
{
	 ...
	 *                        point from world space into "light" space (i.e. changes a point's
	 *                        coordinates from being in relation to the world's axis to being in
	...
	{
		Matrix4f rotation = new Matrix4f().set(cam.transform().quaternion());
		Vector3f forwardVector = cam.transform().getForward(new Vector3f());

		Vector3f toFar = new Vector3f(forwardVector);
		toFar.mul(SHADOW_DISTANCE);
		Vector3f toNear = new Vector3f(forwardVector);
		toNear.mul(this.cam.getNearPlane());
		Vector3f centerNear = toNear.add(cam.transform().position3(), new Vector3f());
		Vector3f centerFar = toFar.add(cam.transform().position3(), new Vector3f());
	...
	protected Vector3f getCenter()
	{
		...
		this.lightViewMatrix.invert(invertedLight);
		Vector4f light4 = invertedLight.transform(cen, new Vector4f());
		return new Vector3f(light4.x(), light4.y(), light4.z());
	}
	...
	{
		Vector4f up4 = rotation.transform(UP, new Vector4f());
		Vector3f upVector = new Vector3f(up4.x(), up4.y(), up4.z());
		Vector3f rightVector = forwardVector.cross(upVector, new Vector3f());
		Vector3f downVector = new Vector3f(-upVector.x, -upVector.y, -upVector.z);
		Vector3f leftVector = new Vector3f(-rightVector.x, -rightVector.y, -rightVector.z);
		Vector3f farTop = centerFar.add(upVector.x * farHeight, upVector.y * farHeight, upVector.z * farHeight, new Vector3f());
		Vector3f farBottom = centerFar.add(downVector.x * farHeight, downVector.y * farHeight, downVector.z * farHeight, new Vector3f());
		Vector3f nearTop = centerNear.add(upVector.x * nearHeight, upVector.y * nearHeight, upVector.z * nearHeight, new Vector3f());
		Vector3f nearBottom = centerNear.add(downVector.x * nearHeight,
	...
	private Vector4f calculateLightSpaceFrustumCorner(Vector3f startPoint, Vector3f direction,
	                                                  float width)
	{
		Vector3f point = startPoint.add(direction.x * width, direction.y * width, direction.z * width, new Vector3f());
	...
	{
		farWidth = (float) (SHADOW_DISTANCE * Math.tan(Math.toRadians(this.cam.getFieldOfView())));
		nearWidth = (float) (this.cam.getNearPlane()
				* Math.tan(Math.toRadians(this.cam.getFieldOfView())));
