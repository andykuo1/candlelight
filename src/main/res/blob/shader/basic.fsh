...
vec3 applyLight(Light light, vec3 surfaceColor, vec3 surfaceNormal, vec3 surfacePos, vec3 surfaceToCamera)
{
    vec3 surfaceToLight;
    ...
    {
        specularCoefficient = pow(max(0.0, dot(surfaceToCamera, reflect(-surfaceToLight, surfaceNormal))), u_shininess);
    }
    vec3 specular = specularCoefficient * u_specular_color * light.intensity;
...
void main()
{
    ...
    vec3 surfaceToCamera = normalize(u_camera_position - surfacePos);
    
    vec3 linearColor = vec3(0, 0, 0);
    for(int i = 0; i < u_light_count; ++i)
    {
        linearColor += applyLight(u_light[i], surfaceColor.rgb, surfaceNormal, surfacePos, surfaceToCamera);
    }
